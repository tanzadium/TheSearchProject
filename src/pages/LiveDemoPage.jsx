...
const Header = ({
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between p-6 pt-[max(1.5rem,env(safe-area-inset-top))]">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col items-start gap-1">
          <span className="text-white font-bold text-3xl drop-shadow-md tracking-wide">
            {username}
          </span>
          <button
            onClick={() => setIsFollowing(!isFollowing)}
            className={`flex items-center gap-2 px-5 py-1.5 rounded-full text-sm md:text-base font-bold transition-all duration-300 shadow-lg ${
              isFollowing
      ...
      >
        <Eye
          size={20}
          className={
            isAutoActive ? "text-green-400 animate-pulse" : "text-white"
          }
        />
        <span className="text-white font-bold text-2xl min-w-[3ch] text-center">
    ...
    <div className="absolute top-28 left-6 z-50 bg-black/90 backdrop-blur-md p-4 rounded-xl border border-white/20 text-white shadow-2xl w-80 animate-slide-up max-w-[90vw]">
      ...
      <div className="flex bg-gray-800 rounded-lg p-1 mb-4">
        <button
          ...
          className={`flex-1 py-2 rounded-md text-sm ${mode === "video" ? "bg-gray-600" : ""}`}
        >
          กล้อง
        </button>
        <button
          onClick={() => setMode("color")}
          className={`flex-1 py-2 rounded-md text-sm ${mode === "color" ? "bg-gray-600" : ""}`}
        >
          สี
        </button>
      </div>
        ...
        {mode === "video" ? (
          devices.map((d) => (
            <button
              ...
              className="block w-full text-left text-sm p-3 border-b border-gray-700 truncate"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <button
                key={c}
                onClick={() => setColor(c)}
                className="flex items-center gap-2 p-2 border border-white/10 rounded"
              >
                <div
                  className={`w-4 h-4 rounded-full ${c === "transparent" ? "border border-gray-500" : ""}`}
                  style={{
                    backgroundColor: c === "transparent" ? "transparent" : c,
                  }}
                />
                {c === "transparent" ? "โปร่งใส" : c}
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div className="hidden md:block absolute bottom-24 right-0 w-48 h-96 pointer-events-none z-0 overflow-hidden">
      {hearts.map((heart) => (
        <div
          key={heart.id}
          className="absolute bottom-0 w-14 h-14 animate-float-up-fade"
          style={{
            left: `${heart.left}%`,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (isAutoFlowing && commentIndex <= MOCK_DATA.length) {
      const currentDelay =
        commentIndex === 1 ? 0 : MOCK_DATA[commentIndex - 2]?.delay || 6000;
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.code === "Space") {
        e.preventDefault();
        setCommentIndex((prev) => (prev < MOCK_DATA.length ? prev + 1 : prev));
      }
  ...
  return (
    ...
    <div className="relative w-full h-[100dvh] overflow-hidden bg-transparent touch-none font-sans">
      ...
      />
      <Header
        username={MOCK_DATA[0]?.username}
        profilePicture={MOCK_DATA[0]?.profilePicture}
        viewerCount={viewerCount}
        onSettingsClick={() => setShowSettings((p) => !p)}
